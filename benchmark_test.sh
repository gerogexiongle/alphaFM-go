...
done | /usr/bin/time -v ./bin/fm_train $TRAIN_PARAMS -m $CPP_MODEL) 2>&1 | tee $BENCHMARK_DIR/cpp_train.log &
CPP_TRAIN_PID=$!
...
(find $TRAIN_DATA_DIR -type f -name "part-*.txt" | sort | while read file; do
    cat "$file" | awk '{printf "%s", $3; for(i=4; i<=NF; i++) printf " %s", $i; printf "\n"}'
done | /usr/bin/time -v ./bin/fm_train $TRAIN_PARAMS -m $GO_MODEL) 2>&1 | tee $BENCHMARK_DIR/go_train.log &
GO_TRAIN_PID=$!
...
(find $TEST_DATA_DIR -type f -name "part-*.txt" | sort | while read file; do
    cat "$file" | awk '{printf "%s", $3; for(i=4; i<=NF; i++) printf " %s", $i; printf "\n"}'
done | /usr/bin/time -v ./bin/fm_predict $PREDICT_PARAMS -m $CPP_MODEL -out $CPP_PREDICTION) 2>&1 | tee $BENCHMARK_DIR/cpp_predict.log &
CPP_PRED_PID=$!
...
(find $TEST_DATA_DIR -type f -name "part-*.txt" | sort | while read file; do
    cat "$file" | awk '{printf "%s", $3; for(i=4; i<=NF; i++) printf " %s", $i; printf "\n"}'
done | /usr/bin/time -v ./bin/fm_predict $PREDICT_PARAMS -m $GO_MODEL -out $GO_PREDICTION) 2>&1 | tee $BENCHMARK_DIR/go_predict.log &
GO_PRED_PID=$!
